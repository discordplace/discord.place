function categoriesValidation(categories) {
  ...
  if (categories.length > config.templateCategories.length) throw new Error(`Categories must not exceed ${config.templateCategories.length}.`);
  if (!categories.some(category => config.templateCategories.find(c => c == category))) throw new Error(`${categories.join(', ')} is not a valid category.`);
