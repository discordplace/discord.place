  ...
  execute: async interaction => {
    ...
    if (group === 'set') {
      if (subcommand === 'invite') {
        if (interaction.user.id !== interaction.guild.ownerId) return interaction.reply(await interaction.guild.translate('commands.shared.errors.server_owner_only'));
        ...
        if (newInviteCode === server?.invite_code?.code) return interaction.followUp(await interaction.guild.translate('commands.server.errors.same_invite_code'));
        ...
        if (newInviteCode === interaction.guild.vanityURLCode) await Server.findOneAndUpdate({ id: interaction.guild.id }, { invite_code: { type: 'Vanity' } });
          ...
          if (!invite) return interaction.followUp(await interaction.guild.translate('commands.server.errors.invalid_invite_code'));
          
          await Server.findOneAndUpdate({ id: interaction.guild.id }, { invite_code: { type: 'Invite', code: newInviteCode } });
        }

        sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.invite_code_updated', { newInviteCode, user: interaction.user.toString() }))
          .catch(() => null);

        return interaction.followUp(await interaction.guild.translate('commands.server.groups.set.subcommands.invite.updated'));
      }

      if (subcommand === 'log') {
        if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.Administrator)) return interaction.reply(await interaction.guild.translate('commands.shared.errors.missing_permissions'));

        const channel = interaction.options.getChannel('channel');
        if (!channel.permissionsFor(interaction.guild.members.me).has(Discord.PermissionFlagsBits.SendMessages)) return interaction.reply(await interaction.guild.translate('commands.server.errors.missing_bot_permissions'));
        ...
        if (logChannel) {
          ...
          return interaction.followUp(await interaction.guild.translate('commands.server.groups.set.subcommands.log.updated'));
        } else {
          ...
          return interaction.followUp(await interaction.guild.translate('commands.server.groups.set.subcommands.log.set'));
        }
      }
      ...
      if (subcommand === 'panel') {
        ...
        if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.Administrator)) return interaction.followUp(await interaction.guild.translate('commands.shared.errors.missing_permissions'));
        ...
        if (!hasPermission) return interaction.followUp(await interaction.guild.translate('commands.server.errors.missing_bot_permissions'));

        const server = await Server.findOne({ id: interaction.guild.id });
        if (!server) return interaction.followUp(await interaction.guild.translate('commands.server.errors.panel_server_not_listed', { link: `${config.frontendUrl}/account` }));
        ...
        if (panel) {
          ...
          sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.panel_channel_updated', { channel: channel.toString(), user: interaction.user.toString() }))
            .catch(() => null);

          return interaction.followUp(await interaction.guild.translate('commands.server.groups.set.subcommands.panel.updated'));
        } else {
          await new Panel({ 
            guildId: interaction.guild.id, 
            channelId: channel.id 
          ...
          return interaction.followUp(await interaction.guild.translate('commands.server.groups.set.subcommands.panel.set'));
        }
      }

      if (subcommand === 'language') {
        const language = interaction.options.getString('language');
        if (!config.availableLocales.some(locale => locale.code === language)) return interaction.reply(await interaction.guild.translate('commands.server.errors.invalid_language'));
        ...
        await sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.language_updated', { user: interaction.user.toString() }))
          .catch(() => null);
          
        return interaction.followUp(await interaction.guild.translate('commands.server.groups.set.subcommands.language.updated'));
      }
    }
    ...
    if (group === 'unset') {
      if (subcommand === 'panel') {
        ...
        const panel = await Panel.findOne({ guildId: interaction.guild.id });
        if (!panel) return interaction.followUp(await interaction.guild.translate('commands.server.errors.panel_not_set'));

        await panel.deleteOne();

        sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.panel_channel_unset', { user: interaction.user.toString() }))
          .catch(() => null);

        return interaction.followUp(await interaction.guild.translate('commands.server.groups.unset.subcommands.panel.unset'));
      }
      ...
      if (subcommand === 'log') {
        ...
        return interaction.followUp(await interaction.guild.translate('commands.server.groups.unset.subcommands.log.unset'));
      }
    }
    ...
    if (group === 'refresh') {
      if (subcommand === 'panel') {
        ...
        sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.panel_message_refreshed', { user: interaction.user.toString() }))
          .catch(() => null);

        return interaction.followUp(await interaction.guild.translate('commands.server.groups.refresh.subcommands.panel.refreshed'));
      }
    }
    ...
    if (group === 'add') {
      if (subcommand === 'reward') {
        if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.Administrator)) return interaction.reply(await interaction.guild.translate('commands.shared.errors.missing_permissions'));
        if (!interaction.guild.members.me.permissions.has(Discord.PermissionFlagsBits.ManageRoles)) return interaction.reply(await interaction.guild.translate('commands.server.groups.add.subcommands.reward.errors.missing_bot_permissions'));

        const role = interaction.options.getRole('role');
        if (role.managed) return interaction.reply(await interaction.guild.translate('commands.server.errors.role_is_managed'));
        if (interaction.guild.members.me.roles.highest.comparePositionTo(role) <= 0) return interaction.reply(await interaction.guild.translate('commands.server.errors.role_is_higher'));

        const requiredVotes = interaction.options.getInteger('required-votes');
        if (requiredVotes < 1) return interaction.reply(await interaction.guild.translate('commands.server.groups.add.subcommands.reward.errors.invalid_votes'));

        if (!interaction.deferred && !interaction.replied) await interaction.deferReply();

        const foundServer = await Server.findOne({ id: interaction.guild.id });
        if (!foundServer) return interaction.followUp(await interaction.guild.translate('commands.server.errors.server_not_listed', { link: `${config.frontendUrl}/account` }));
        ...
        if (foundRewards.length >= (ownerHasPremium ? 20 : 5)) return interaction.followUp(await interaction.guild.translate('commands.server.groups.add.subcommands.reward.errors.max_rewards_reached', { count: ownerHasPremium ? 20 : 5 }));

        const rewardExists = foundRewards.some(reward => reward.role.id === role.id);
        if (rewardExists) return interaction.followUp(await interaction.guild.translate('commands.server.groups.add.subcommands.reward.errors.reward_exists'));
        ...
        sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.vote_reward_added', { role: role.toString(), requiredVotes, user: interaction.user.toString() }))
          .catch(() => null);

        return interaction.followUp({
          content: await interaction.guild.translate('commands.server.groups.add.subcommands.reward.reward_added', { role: role.toString(), requiredVotes }),
    ...
    if (group === 'remove') {
      if (subcommand === 'reward') {
        ...
        if (!reward) return interaction.reply(await interaction.guild.translate('commands.server.groups.remove.subcommands.reward.errors.reward_not_found'));

        if (!interaction.deferred && !interaction.replied) await interaction.deferReply();

        await reward.deleteOne();

        sendLog(interaction.guild.id, await interaction.guild.translate('commands.server.logging_messages.vote_reward_removed', { role: `<@&${roleId}>`, user: interaction.user.toString() }))
          .catch(() => null);

        return interaction.followUp(await interaction.guild.translate('commands.server.groups.remove.subcommands.reward.reward_removed'));
      }
    }
    ...
    if (group === 'list') {
      if (subcommand === 'rewards') {
        ...
        if (!rewards.length) return interaction.followUp(await interaction.guild.translate('commands.server.groups.list.subcommands.rewards.no_rewards'));

        const embed = new Discord.EmbedBuilder()
          .setColor('Random')
          .setAuthor({ name: await interaction.guild.translate('commands.server.groups.list.subcommands.rewards.embed.title', { serverName: interaction.guild.name }), iconURL: interaction.guild.iconURL() })
          .setDescription(`${await interaction.guild.translate('commands.server.groups.list.subcommands.rewards.embed.description', { count: rewards.length })}

${(await Promise.all(rewards.sort((a, b) => a.required_votes - b.required_votes).map(async reward => await interaction.guild.translate('commands.server.groups.list.subcommands.rewards.embed.item', { role: `<@&${reward.role.id}>`, requiredVotes: reward.required_votes })))).join('\n\n')}`);
  ...
  autocomplete: async interaction => {
    ...
    if (group === 'remove') {
      if (subcommand === 'reward') {
        ...
        return interaction.customRespond(await Promise.all(rewards.map(async reward => ({ name: await interaction.guild.translate('commands.server.groups.remove.subcommands.reward.autocomplete.item', { role: `${interaction.guild.roles.cache.get(reward.role.id)?.name || reward.role.id}`, requiredVotes: reward.required_votes }), value: reward.role.id }))));
      }
    }
  }
};