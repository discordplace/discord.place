...
export default function UserBanner({ id, hash, format, size, className, motionOptions, ...props }) {
  ...
  function getUrl(id, hash) {
    return `https://cdn.discordapp.com/banners/${id}/${hash}.${format || (hash?.startsWith('a_') ? 'gif' : 'webp')}?size=${size || 256}`;
  }
  ...
  useEffect(() => {
    ...
    async function fetchImage() {
      try {
        const response = await axios.get(getUrl(id, hash));
  ...
  return (
    <MotionImage
      ...
      className={className}
      unoptimized={format === 'gif' || hash?.startsWith('a_')}
      placeholder={DEFAULT_BANNER_BASE64}
      {...motionOptions}
      {...props}
    />
  );
