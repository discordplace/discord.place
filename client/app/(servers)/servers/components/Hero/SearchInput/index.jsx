...
export default function SearchInput() {
  ...
  const { loading, search, fetchServers } = useSearchStore(useShallow(state => ({ 
  ...
  function validateValue(throwError = true) {
    ...
    if (!value) return returnError('Please enter a valid search query to find servers.');

    const trimmedValue = value.trim();
    if (trimmedValue.length <= 0) return returnError('Please enter a valid search query to find servers.');
  ...
  return (
    <div className='flex mt-8 gap-x-2'>
      <div className='relative flex items-center w-full'>
        <motion.input
          type="text"
          placeholder="Search for a server by id, description, keywords, or category..."
          value={value}
          disabled={loading}
          ...
          onKeyUp={event => {
            if (event.key === 'Enter') {
              ...
              if (validatedValue) fetchServers(validatedValue);
            }
          }}
