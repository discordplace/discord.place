    ...
    <header className="px-4 bg-purple-800 lg:px-0">
      <div className='flex mx-auto lg:max-w-3xl'>
        {HEADER_LINKS.map(({ name, href }) => (
          <Link
            className={cn(
              'p-4 text-sm font-medium text-primary first:pl-0 last:pr-0',
              pathname === href ? 'cursor-default' : 'text-primary/60 hover:text-primary/80'
            )}
            key={name}
            href={href}
          >
      ...
      >
        ...
        >
          ...
          {HEADER_LINKS.map(({ name, href }) => (
            <Link
              className={cn(
                'py-2 text-lg font-medium text-primary',
                pathname === href ? 'cursor-default' : 'text-primary/60 hover:text-primary/80'
              )}
              key={name}
              href={href}
...
function HomeHeader() {
  return (
    <div className='px-4 bg-purple-900 lg:px-0'>
      <div className='flex mx-auto space-x-24 lg:max-w-3xl py-7'>
        <div className='flex flex-col space-y-2'>
          ...
          <p className='max-w-lg text-primary/90'>
